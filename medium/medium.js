...
for (let i = 0; i < squares.length; i++) { //...(1)[add initial colors to squares]
    ...
    squares[i].addEventListener("click", function () { //setting the click event
        ...
        if (clickedColor === pickedColor) { //compare color to pickedColor
            ...
            newColors();
            if (score >= 100) {
                window.location.href = "https://subhambakshi.github.io/Guess-the-Color---Game/Fillers/Win2.html"
            }
        } else {
    ...
    function countdown() {
        setInterval(function () {
            if (timeLeft <= 0) {
                ...
                window.location.href = "https://subhambakshi.github.io/Guess-the-Color---Game/Fillers/LostMedium.html";
            }
